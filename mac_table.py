...
def show_mac_table(host: str, platform_id: str, credential_id: str):
    """
    show_mac_table summary
        Logs into a network device and runs the command "show mac address-table".  The command
    ...
    # add to the dictionary the result of the "show mac address-table" command
    temp["neighbors"] = ssh_client.session.send_command("show mac address-table", strip_command=True, strip_prompt=True, use_textfsm=True)
