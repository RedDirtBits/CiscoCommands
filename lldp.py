...
def show_lldp_neighbors(host: str, platform_id: str, credential_id: str):
    """
    show_lldp_neighbors summary
        Logs into a network device and runs the command "show lldp neighbors".  The command
    ...
    # add to the dictionary the result of the "show lldp neighbors" command
    temp["neighbors"] = ssh_client.session.send_command("show lldp neighbors", strip_command=True, strip_prompt=True, use_textfsm=True)
